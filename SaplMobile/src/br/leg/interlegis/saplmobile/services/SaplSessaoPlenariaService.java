...
public class SaplSessaoPlenariaService extends Service {
	...
	private int contadorProcessos = 600;
	private int contadortimers= 0;

	private float timeListaMaterias = 30;
	private float timeListaMaximo = 180;
	private int timeListaMateriasFixo = 30;
	...
	private void startservice() {
		...
		timer.scheduleAtFixedRate( new TimerTask() {
			...
			public void run() { 
				...
				Log.d("CMJ", "timer"+((int)getInstance().timeListaMaterias)+": "+contTimer);


				if (contadorProcessos % 60 == 0) { //Atualizar Lista de Datas 

					Log.d("CMJ", "timer: "+contTimer);

					NetworkUtil.isConnected();

					Log.d("CMJ", "servico LS: "+contadorProcessos);
					try {
				...
				else if (contadorProcessos % ((int)(timeListaMaterias-1)) == 0) { //Atualizar Lista de Matérias


					Log.i("CMJ", "timer"+((int)getInstance().timeListaMaterias)+": "+contTimer);
					NetworkUtil.isConnected();
				...
				if (contadorProcessos % ((int)(timePontoDownloadFileMaterias-1)) == 0) { //Download Materias
