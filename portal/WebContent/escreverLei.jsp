...
<%@page import="br.gov.go.camarajatai.sislegisold.suport.Suport"%>
		...
		while (collection.hasNext()) {
			...
			if (lei.completo && isIntranet  && !novalei) {
				%>
				<hr><span style="color: #ff0000;">Código do dispositivo alterador: <%=il.getId() %></span>
				<%
			}
		%>
...
<%	
		...
		<tr>
			<td>
				<iframe width="100%" height="400px" frameborder="0" src="<%=Urls.urlPortalSislegisBase %><%=lei.getContent_type().indexOf("pdf")!=-1?"downloadFile.pdf":"downloadFile"%>?sv=2&id=<%=lei.getId() %>"></iframe>
			</td>	
		</tr>
		</table>
		...
		display: block;
		font-weight: bold;
		">Hash SHA512:</span><%=lei.getHash_arqdigital()%> </div>
