	    ...
	    <legend> Escolher outro Mês </legend>
				...
				<label class="campoLabel">
					...
					<select name="ano" id="selectAno">
							<%
							for (int i = data.get(GregorianCalendar.YEAR); i > data.get(GregorianCalendar.YEAR)-5; i-- ){
							%>
			...
			<%} %>
				...
				<div class="iface_titulo"><span style="font-size:11pt;">Salários Abertos. Mês: <%=mesinformado %> / <%=anoinformado %></span></div>
					...
					while (it.hasNext()) {
						...
						%>
						<tr>
						<td><a href="olerite.do?ano=<%=anoinformado+"&mes="+mesinformado+"&matricula="+f.getMatricula()+"&action=olerite_func"+(prov_expandir?"&p=1":"") %>" title="Clique para ver remuneração detalhada"><%=f.getNome().trim() %></a></td>
						<td><a href="olerite.do?ano=<%=anoinformado+"&mes="+mesinformado+"&matricula="+f.getMatricula()+"&action=olerite_func"+(prov_expandir?"&p=1":"") %>" title="Clique para ver remuneração detalhada"><%=f.getCargo().trim().toLowerCase()+" ("+f.getTipoAdmissao().trim()+")" %></a></td>
						<td style="text-align:right;"><a href="olerite.do?ano=<%=anoinformado+"&mes="+mesinformado+"&matricula="+f.getMatricula()+"&action=olerite_func" %>" title="Clique para ver remuneração detalhada"><%=f.getLiquidacao().get(0).getValorLiquido() %></a></td>
						</tr>
						<%
					}				
